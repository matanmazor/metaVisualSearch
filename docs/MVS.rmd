...
note              : |
 Anonymized data, analysis scripts, experiment demos, and stimulus materials are available at [github.com/matanmazor/metaVisualSearch](https://github.com/matanmazor/metaVisualSearch). Experiments 2, 4, 5 and 6 were pre-registered prior to data collection. Pre-registration documents are available at [https://osf.io/e48n5/registrations](https://osf.io/e48n5/registrations). M.M. was supported by a UCL Bogue Fellowship. This research was funded in whole, or in part, by the Wellcome Trust [Grant 203147/Z/16/]. For the purpose of open access, the author has applied a CC BY public copyright licence to any Author Accepted Manuscript version arising from this submission. The authors have no conflicting interests to disclose. We thank two anonymous reviewers for their useful comments and input, Noam and Bat-Sheva Mazor for help with developing the bonus structure, and Felix Henninger for assistance and guidance in programming the online experiments. We would especially like to thank signed reviewer Jeremy Wolfe for invaluable feedback on previous drafts of this paper, and for suggesting we run Experiments 5 and 6. Correspondence concerning this article should be addressed to Matan Mazor, WC1E 7HX, London UK. E-mail: mtnmzor@gmail.com. 
...
In Experiments 1 and 2 we used stimuli that lend themselves to a categorical distinction between parallel and serial search: simple geometrical shapes of different colors and orientations. We asked whether participants' internal models of visual search predict which search displays demand serial deployment of attention and which don't. Critically, participants gave their search time estimates before they were asked to perform searches involving these or similar stimuli, so their search time estimates reflected prior beliefs about search efficiency. Experiment 2 was designed to replicate and generalize the results of Exp. 1 to a new stimulus dimension (orientation) and distractor set sizes. Our hypotheses and analysis plan for Experiment 2, based on the results of Experiment 1, were pre-registered prior to data collection (pre-registration document: [doi.org/10.17605/OSF.IO/2DPQ9](https://doi.org/10.17605/OSF.IO/2DPQ9)). Raw data, experiment demos, and full analysis scripts are available at [github.com/matanmazor/metaVisualSearch](https://github.com/matanmazor/metaVisualSearch).
...
## Procedure
The study was built using the Lab.js platform [@henninger2019lab] and hosted on a JATOS server [@lange2015just]. Demo versions of all four experiments are available at [github.com/matanmazor/metaVisualSearch](https://github.com/matanmazor/metaVisualSearch).
...
knitr::include_graphics("figures/methods2bold.png")
```
...
knitr::include_graphics("figures/results3.png")
...
#after some manual tweaks
knitr::include_graphics('figures/alignment_edited.png');
```
...
p <- long_data %>%
  ...
  geom_flat_violin(position=position_nudge(x=.2,y=0),adjust=1, size=1) +
  geom_point(position=position_jitter(width=.1),size=2,alpha=0.2) +
  ylab('Spearman correlation') +
  # theme_cowplot() +
  guides(fill=FALSE,color=FALSE) +
  geom_boxplot(aes(x=as.numeric(condition),y=mycor),
               outlier.shape = NA, 
               alpha=0.3, 
               width=0.2, 
               color='black',
...
e6 <- e6%>%filterDataFrame();

```



Accuracy in the visual search task was high (`r e6$acc%>%t.test()%>%apa_print()%>%'$'(estimate)`). Error trials and visual search trials that took shorter than `r min_RT*1000` milliseconds or longer than `r max_RT` seconds were excluded from all further analysis. Participants were excluded if more than `r max_exclude`\% of their trials were excluded based on the aforementioned criteria, leaving `r e6$N` participants for the main analysis of Experiment 6. 


```{r MVS-e6-search-slopes, include=FALSE, cache=TRUE}
...
e6 <- e6%>%getSlopes()
e6$search_slopes_anova <- aov_car(estimate ~ search_type + Error(Subj_id/search_type), data=e6$search_slopes)

e6$slope_diffs <- e6$search_slopes %>%
  dplyr::select(Subj_id, search_type, estimate) %>%
  spread(search_type, estimate) %>%
  mutate(diff3 = `3in82`-`3inAH`,
         diffE = EinAH-Ein82)
      

```

```{r MVS-e6-estimation, include=FALSE, cache=TRUE}

analyzeEstimatesExp6 <- function(e) {
  ...
  e$est_slopes <- e$filtered_df %>%
  group_by(Subj_id,search_type,set_size)%>%
  summarise(estimates=mean(estimates))%>%
  group_by(Subj_id,search_type) %>%
  spread(set_size,estimates,sep='_') %>%
  mutate(estimate=(set_size_18-set_size_6)/12)
...
e6 <- e6 %>%
  analyzeEstimatesExp6() 

e6$est_slopes_anova <- aov_car(estimate ~ search_type + Error(Subj_id/search_type), data=e6$est_slopes)

# Spearman correlations
e6$RT_est_corr <- e6$avgd %>%
  group_by(Subj_id) %>%
  summarize(COR = cor(RT,estimates,method = 'spearman',use='complete.obs'),
            COR6 = cor(RT[set_size==6], estimates[set_size==6],method = 'spearman',use='complete.obs'),
            COR18 = cor(RT[set_size==18], estimates[set_size==18],method = 'spearman',use='complete.obs'))

```

```{r MVS-e6-by-condition, include=FALSE, cache=TRUE}

e6$search_slopes <- e6$search_slopes %>%
  mutate(condition = ifelse(search_type %in% c('3in82','EinAH'), 
                            'baseline', ifelse(search_type %in% c('Ein82', '3inAH'), 
                                   'semantic','visual')),
         target = substr(as.character(search_type),0,1),
         distractors = ifelse(search_type %in% c('3in82','Ein82','3inUpright'), 
                            'numbers', 'letters'))

e6$search_slopes_by_condition <- e6$search_slopes %>%
  group_by(Subj_id,condition)%>%
  summarise(estimate=mean(estimate)) %>%
  spread(condition,estimate) %>%
  mutate(semantic_diff=semantic-baseline,
         visual_diff = visual-baseline)

e6$search_slopes_by_target <- e6$search_slopes %>%
  group_by(Subj_id,target)%>%
  summarise(estimate=mean(estimate)) %>%
...
e6$search_slopes_by_distractors <- e6$search_slopes %>%
  group_by(Subj_id,distractors)%>%
  summarise(estimate=mean(estimate)) %>%
  spread(distractors,estimate) %>%
  mutate(diff=letters-numbers)

e6$est_slopes <- e6$est_slopes  %>%
  mutate(condition = ifelse(search_type %in% c('3in82','EinAH'), 
                            'baseline', ifelse(search_type %in% c('Ein82', '3inAH'), 
                                   'semantic','visual')),
         target = substr(as.character(search_type),0,1),
...
e6$est_slopes_by_condition <- e6$est_slopes %>%
  group_by(Subj_id,condition)%>%
  summarise(estimate=mean(estimate)) %>%
  spread(condition,estimate) %>%
  mutate(semantic_diff=semantic-baseline,
...
e6$est_slopes_by_target <- e6$est_slopes %>%
  group_by(Subj_id,target)%>%
  summarise(estimate=mean(estimate)) %>%
  spread(target,estimate) %>%
  mutate(diff=E-`3`)
...
e6$est_slopes_by_distractors <- e6$est_slopes %>%
  group_by(Subj_id,distractors)%>%
  summarise(estimate=mean(estimate)) %>%
  spread(distractors,estimate) %>%
  mutate(diff=letters-numbers)
...
e6$avgd_by_condition <- e6$avgd  %>%
  ...
  summarise(RT=mean(RT),
            estimates=mean(estimates),
            timeToEstimate=mean(timeToEstimate))

e6$median_long <- e6$avgd_by_condition %>%
...
e6$sem_long <- e6$avgd_by_condition %>%
  group_by(condition,set_size) %>%
  summarize_at(vars('RT','estimates'),bootstrap_error)%>%
  mutate(RT=RT*1000,
         estimates=estimates*1000)%>%
...
e6$median_figure_facet <- e6$median_long %>% 
  merge(e6$sem_long) %>%
  mutate(measure=factor(measure, levels=c('RT','estimates'), labels=c('RT','estimate')))%>%
  ggplot(aes(x=set_size,y=median,color=condition,pch=condition, fill=condition))+
  geom_errorbar(
    aes(ymin = median-sem,ymax = median+sem),width=2)+
  ...
  facet_grid(cols=vars(measure))+
    guides(size=FALSE) +
    scale_x_continuous(breaks=e6$median_long$set_size%>%unique())+
    scale_fill_manual(values = c('black',"#e41a1c","#377eb8","#4daf4a")) + 
    scale_color_manual(values = c('black',"#e41a1c","#377eb8","#4daf4a")) +
  theme(legend.position='na')

ggsave('figures/exp6medians.png',e6$median_figure_facet,width=5, height=2.5);

e6$median_long_by_st <- e6$avgd %>%
group_by(search_type,set_size) %>%
summarize_at(vars('RT','estimates'),median, na.rm=TRUE)%>%
  mutate(RT=RT*1000,
         estimates=estimates*1000)%>%
...
e6$median_by_st_figure_facet <- e6$median_long_by_st %>% 
  merge(e6$sem_long_by_st) %>%
  mutate(condition = ifelse(search_type %in% c('3in82','EinAH'), 
                            'baseline', ifelse(search_type %in% c('Ein82', '3inAH'), 
                                   'semantic','visual')),
         target=substr(as.character(search_type),0,1),
         measure=factor(measure, levels=c('RT','estimates'), labels=c('RT','estimate')))%>%
  ggplot(aes(x=set_size,y=median,color=condition,pch=condition, fill=condition))+
  geom_errorbar(
    aes(ymin = median-sem,ymax = median+sem),width=2)+
  geom_point(alpha=0.5,size=6)+
  geom_path(size=1)+
    ...
    labs(x='set size',
         y='median search time (ms)') +
  facet_grid(cols=vars(measure),rows=vars(target))+
    guides(size=FALSE) +
    scale_x_continuous(breaks=e6$median_long$set_size%>%unique())+
    scale_fill_manual(values = c('black',"#e41a1c","#377eb8","#4daf4a")) + 
    scale_color_manual(values = c('black',"#e41a1c","#377eb8","#4daf4a")) +
...
e6$median_by_dist_figure_facet <- e6$median_long_by_st %>% 
  merge(e6$sem_long_by_st) %>%
  mutate(condition = ifelse(search_type %in% c('3in82','EinAH'), 
                            'baseline', ifelse(search_type %in% c('Ein82', '3inAH'), 
                                   'semantic','visual')),
  ...
  geom_path(size=1)+
    ...
    labs(x='set size',
         y='median search time (ms)') +
  facet_grid(cols=vars(measure),rows=vars(distractors))+
    guides(size=FALSE) +
    scale_x_continuous(breaks=e6$median_long$set_size%>%unique())+
    scale_fill_manual(values = c('black',"#e41a1c","#377eb8","#4daf4a")) + 
    scale_color_manual(values = c('black',"#e41a1c","#377eb8","#4daf4a")) +
...
ggsave('figures/exp6medians_by_dist.png',e6$median_by_dist_figure_facet,width=5, height=5);


# e6$median_long_by_st %>% 
#   merge(e6$sem_long_by_st) %>%
#   mutate(distractors = ifelse(search_type %in% c('3in82','Ein82', '3inUpright'), 
#                             'numbers', 'letters'),
#          target=substr(as.character(search_type),0,1),
#          upright=ifelse(search_type %in% c('3inUpright','EinUpright'),'upright','tilted'),
#          measure=factor(measure, levels=c('RT','estimates'), labels=c('RT','estimate'))) %>%
#   ggplot(aes(x=set_size,y=median,color=distractors,pch=target, fill=distractors, linetype=upright, label=ifelse(target=='3','3','')))+
#   geom_errorbar(
#     aes(ymin = median-sem,ymax = median+sem),width=2)+
#   geom_point(alpha=0.5,size=6)+
#   geom_path(size=1)+
...
#   theme(legend.pos = 'na')
# 
# ggsave('figures/exp6medians_unpacked.png',width=5, height=2.5);
knitr::include_graphics("figures/exp6.png")

```


Search slopes were significantly different for the six searches (`r apa_print(e6$search_slopes_anova)$full_result$search_type`; see Fig. \@ref(fig:MVS-exp6-fig)). Specifically, search slopes were significantly shallower when the target and distractors were of different semantic categories (`r round(e6$search_slopes_by_condition$semantic%>%mean(na.rm=T)*1000)` ms/item) compared to the baseline condition (`r round(e6$search_slopes_by_condition$baseline%>%mean(na.rm=T)*1000)` ms/item; `r apa_print(e6$search_slopes_by_condition$semantic_diff%>%t.test())$statistic`). Visual dissimilarity produced an even stronger effect on estimation slopes (`r round(e6$search_slopes_by_condition$visual%>%mean(na.rm=T)*1000)` ms/item in the visual dissimilarity condition versus `r round(e6$search_slopes_by_condition$baseline%>%mean(na.rm=T)*1000)` ms/item in the baseline condition; `r apa_print(e6$search_slopes_by_condition$visual_diff%>%t.test())$statistic`). Finally, searching among number distractors was overall harder than searching among letter distractors (`r apa_print(e6$search_slopes_by_distractors$diff%>%t.test())$statistic`; See Fig. \@ref(fig:exp6-full-fig) for distractor-specific effects). 

Prospective search time estimates were significantly correlated with actual search times (within-subject Spearman correlations: `r apa_print(e6$RT_est_corr$COR%>%t.test())$full_result`). Similar to search slopes, estimation slopes were also significantly different for the six searches (`r apa_print(e6$est_slopes_anova)$full_result$search_type`). However, unlike true search times, here we find no significant differences as a function of semantic dissimilarity (`r round(e6$est_slopes_by_condition$semantic%>%mean(na.rm=T)*1000)` ms/item for both the semantic dissimilarity and baseline conditions; `r apa_print(e6$est_slopes_by_condition$semantic_diff%>%t.test())$statistic`), or visual similarity (`r round(e6$est_slopes_by_condition$visual%>%mean(na.rm=T)*1000)` ms/item and `r round(e6$est_slopes_by_condition$baseline%>%mean(na.rm=T)*1000)` ms/item in the visual dissimilarity and baseline conditions; `r apa_print(e6$est_slopes_by_condition$visual_diff%>%t.test())$statistic`). Subjects were however sensitive to the main effect of distractor type on search time, producing steeper estimation slopes for numbers (`r round(e6$est_slopes_by_distractors$numbers%>%mean(na.rm=T)*1000)` ms/item) than for letters  (`r round(e6$est_slopes_by_distractors$letters%>%mean(na.rm=T)*1000)` ms/item; `r apa_print(e6$est_slopes_by_distractors$diff%>%t.test())$statistic`; See Fig. \@ref(fig:exp6-full-fig)). Together, target-distractor similarity along the manipulated dimensions had significant effects on search difficulty, but we found no trace for these effects in search time estimates. 
...
<!-- Finally, it remains an open question whether people’s internal models of visual search are represented as an approximate probabilistic model or alternatively as a list of rules. For example, an alternative description of the above model^[proposed by signed Reviewer Jeremy Wolfe] consists of the following three rules: people believe that searches are harder when 1) there are many distractors, 2) the distractors are similar to the target, and 3) the distractors are different from each other. A list of rules may be easier for subjects to represent than a probabilistic model, but this comes at the expense of being less flexible in predicting search times in unfamiliar settings. More work is needed to determine subjects' ability to mentally simulate their own attention and the role of such simulation in predicting visual search behaviour, noting that simulation- and rule-based accounts are not mutually exclusive [@hegarty2004mechanical; @bass2021partial].  -->

### Conclusion

Across six experiments, we observed the following patterns in prospective search time estimates: First, estimates correlated with search times (Exp. 1-6). Second, estimates reflected knowledge of the set-size effect (Exp. 1, 2, 5 and 6), and were biased to assume a set-size effect even in searches that are in fact parallel (Exp. 1, 2). Third, estimates were sensitive to the relative efficiency of different searches (Exp. 1-6), even for complex, unfamiliar stimuli (Exp. 3, 4). Fourth, estimates were sensitive to the assignment of stimuli to target and distractor roles (Exp. 4, 5), but did not show reliable search asymmetries for basic visual features (Exp. 5). Finally, estimates were not sensitive to direct manipulation of target-distractor similarity along visual and semantic dimensions (Exp. 6). 
